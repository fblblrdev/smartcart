...
serve(async (req) => {
  ...
  try {
    ...
    const resellerEmail = Deno.env.get('RESELLER_EMAIL');
    ...
    const itemsHtml = items.map((item: any) => `
      <tr>
        ...
        <td style="padding:8px;border:1px solid #ddd;text-align:right">₹${((item.price ?? 0) * item.quantity).toFixed(2)}</td>
      </tr>
    `).join('');
    ...
    const html = `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto">
        ...
        <h3 style="text-align:right">Total Amount: <span style="color:#1976d2">₹${Number(order.total_amount).toFixed(2)}</span></h3>

        <p style="color:#666;font-size:13px;margin-top:24px">
          This is an automated notification from SmartCart. Please contact the customer to confirm payment and delivery.
        </p>
      </div>
    `;
